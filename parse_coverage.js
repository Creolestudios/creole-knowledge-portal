...
const modifiedFiles = fs.readFileSync('scratch/modified.txt', 'utf8').split('\n').filter(Boolean).map(f => f.trim());
...
for (const fileChunk of files) {
  ...
  const isModified = modifiedFiles.some(mf => filePath.endsWith(mf) || mf.endsWith(filePath));
  ...
  const linesFound = linesFoundMatch ? Number.parseInt(linesFoundMatch[1]) : 0;
  const linesHit = linesHitMatch ? Number.parseInt(linesHitMatch[1]) : 0;
