    ...
    <div className="min-h-screen bg-[#f8f9fa] flex">
      ...
      <aside className="w-72 bg-[#0a0a0a] text-white flex flex-col p-8 hidden md:flex border-r border-zinc-800 relative overflow-hidden">
        ...
        <nav className="flex-1 space-y-1 relative z-10">
          ...
          >
            <Home size={20} className={activeTab === 'home' ? '' : 'group-hover:scale-110 transition-transform'} />
            <span className={activeTab === 'home' ? 'font-semibold text-sm' : 'font-medium text-sm'}>Morning Brief</span>
          </button>
          ...
          >
            <UserCircle size={20} className={activeTab === 'profile' ? '' : 'group-hover:scale-110 transition-transform'} />
            <span className={activeTab === 'profile' ? 'font-semibold text-sm' : 'font-medium text-sm'}>Profile</span>
            
            {/* Completion Badge */}
            <div className={`absolute right-4 w-1.5 h-1.5 rounded-full ${profileComplete ? 'bg-emerald-500' : 'bg-brand animate-pulse'}`} />
          </button>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-20 bg-white border-b border-zinc-200 px-10 flex items-center justify-between sticky top-0 z-20">
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-4">
              ...
              <button 
                onClick={() => setActiveTab('profile')}
                className={`w-11 h-11 rounded-xl flex items-center justify-center border transition-all shadow-sm ${
                  activeTab === 'profile' 
        ...
        {/* Dynamic Tab Bar for Mobile or Content Consistency */}
        <div className="md:hidden bg-white border-b border-zinc-100 px-6 py-2 flex items-center gap-4">
          <button 
            onClick={() => setActiveTab('home')}
            className={`px-4 py-2 text-xs font-black uppercase tracking-widest border-b-2 transition-all ${
              activeTab === 'home' ? 'border-brand text-brand' : 'border-transparent text-zinc-400'
            }`}
          >
            Briefing
          </button>
          <button 
            onClick={() => setActiveTab('profile')}
            className={`px-4 py-2 text-xs font-black uppercase tracking-widest border-b-2 transition-all ${
              activeTab === 'profile' ? 'border-brand text-brand' : 'border-transparent text-zinc-400'
            }`}
          >
            Profile
          </button>
